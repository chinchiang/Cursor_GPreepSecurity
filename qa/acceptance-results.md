...
| 範圍 | 裁決 | 狀態 | 說明 |
| --- | --- | --- | --- |
| 1. 來源歸屬 | **失敗** | 實際執行 | 研究登錄誠實；Skills 全平台 ID 對調並升格為 gartner-stated |
| 2. 平台能力 | **條件通過（文件層）** | 實際執行 | 有來源 URL 與「未做帳號實測」聲明；DeepSeek v4 未編造單一規格。端到端 `尚未驗證` |
| 3. Skills 可操作性 | **失敗** | 實際執行 | 五平台皆有可複製檔；`validate_contract.py` 通過；但來源 ID 與契約信封不一致 |
...
| [ ] | 每段重要主張行內追溯 `source_id` | 失敗 | 實際執行 | Gartner／第三方主張多有 ID。大量 `[project-framework]` 段落只有分類、同行無 `SRC-2026-900`。Skills 引用與登錄不一致（ISS-001，責任 B） |
...
| [ ] | 未把專案評分／成熟度歸屬 Gartner（**全產品含 Skills／契約範例**） | 失敗 | 實際執行 | 研究與網站有免責。契約 6.1 範例仍把 SRC-2025-001 標 `gartner-stated`+`supported`（ISS-002）。Skills 標題「可引用的 gartner-stated 句子」升格轉載（ISS-001） |
